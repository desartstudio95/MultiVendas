    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (activeCategory !== 'Todos') {
          q = query(collection(db, 'products'), where('category', '==', activeCategory));
        } else {
        ...
        if (activeCategory !== 'Todos') {
          firestoreProducts = firestoreProducts.filter(p => p.status === 'active' || p.status !== 'disabled');
        }

        setProducts(firestoreProducts);
      } catch (error: any) {
        if (error?.message?.includes('permissions')) {
          toast.error("Erro de Permissão no Firebase (CategoriesPage)");
        }
        console.error("Fetch error [CategoriesPage]:", error);
      } finally {
        setLoading(false);
      }
    };
